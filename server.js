...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && 
    ...
    // if body isnt supplied or username doesn't exist or article with supplied ID doesn't exist
    response.status = 400;
  }
...
function updateComment(url, request){
  ...
  if (!requestComment || !requestComment.body) response.status = 400;
  else if (!savedComment) response.status = 404;
  else {
...
const { data } = require('react/lib/ReactDOMFactories');
